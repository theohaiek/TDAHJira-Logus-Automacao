...
| O trilho do quadro | `views/quadro.js` é o arquivo-eixo; a fileira sai de `escoposDoCarrossel()` e o escopo de cada cena de `escopoDeCena()`, ambos em `store.js`. **`escopoDeCena()` nunca devolve `null`**: parâmetro padrão só vale para `undefined`, e um `null` ali joga `TypeError` na desestruturação e deixa o quadro em tela branca. Escrever `scrollLeft` dispara `scroll`, e `scroll` conclui a navegação, que chama `emit()`, que remonta e reposiciona — o contador `programatico` é o que impede esse laço de girar para sempre |
| O Hoje | Ele não é uma tela: `rota()` desvia para `abrirHoje()` e normaliza o hash. `state.view` nunca vale `"hoje"`, e `VIEWS.hoje` existe só pelo item de navegação, o ícone e a contagem. Não tente tratar cada botão que leva até ele: são seis caminhos, e um deles é o logotipo da barra lateral |
