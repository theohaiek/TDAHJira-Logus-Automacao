...
import { openDb, getSetting, setSetting, one, nowIso } from "./db.js";
...
if (primeiraVez || !getSetting("install_id")) {
  setSetting("install_id", newInstallId());
  setSetting("version", "1.0.0");
...
if (flag("setup") || (primeiraVez && !flag("demo"))) {
  const total = one("SELECT COUNT(*) AS n FROM users").n;
  if (total === 0) {
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === "api" || path.startsWith("api/")) {
      ...
      const user = userFromToken(cookies[COOKIE]);
    ...
    // Os arquivos de marca vivem fora de web/ porque são material da empresa,
    // não código da interface.
    if (alvo.startsWith("assets/")) {
      if (serveStatic(res, ROOT, alvo)) return;
    }
